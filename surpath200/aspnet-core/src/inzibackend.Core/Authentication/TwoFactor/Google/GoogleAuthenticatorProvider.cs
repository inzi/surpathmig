...
namespace inzibackend.Authentication.TwoFactor.Google;

public class GoogleAuthenticatorProvider : inzibackendServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
{
    ...
    public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
    {
        ...
        var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("inzibackend", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
