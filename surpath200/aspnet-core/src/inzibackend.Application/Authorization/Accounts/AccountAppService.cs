...
using inzibackend.Authentication.PasswordlessLogin;
using inzibackend.Authorization.PasswordlessLogin;

namespace inzibackend.Authorization.Accounts;

public class AccountAppService : inzibackendAppServiceBase, IAccountAppService
{
    ...
    private async Task SendSmsPasswordlessCode(string phoneNumber)
    {
        ...
        var message = string.Format(L("PasswordlessLogin_SmsMessage", inzibackendConsts.ProductName, code));
