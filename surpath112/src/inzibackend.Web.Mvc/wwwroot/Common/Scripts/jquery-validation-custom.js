  ...
  $.validator.addMethod(
    ...
    $.validator.addMethod(
        ...
        function (value, element) {
            ...
            var pn = value.replace(/[^0-9]/gi, '');
            //return new RegExp('[1-9]{1}[0-9]{9}').test(pn);
            var _result = new RegExp('[1-9]{1}[0-9]{9}').test(pn);
            _result = pn.includes('000000') ? false : _result; // too many zeros
            _result = new RegExp('(\\d)\\1{8}').test(pn.slice(-9)) ? false : _result;  // last 9 digits the same
