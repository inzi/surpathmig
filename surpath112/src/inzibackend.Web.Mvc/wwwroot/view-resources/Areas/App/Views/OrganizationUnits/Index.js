  ...
  $(function () {
    ...
    var _permissions = {
      ...
      manageMembers: abp.auth.hasPermission('Pages.Administration.OrganizationUnits.ManageMembers'),
        ...
        manageDepartments: abp.auth.hasPermission('Pages.Administration.OrganizationUnits.ManageDepartments'),
    };
    ...
    var _addRoleModal = new app.ModalManager({
      ...
      var _addDepartmentModal = new app.ModalManager({
          viewUrl: abp.appPath + 'App/OrganizationUnits/AddDepartmentModal',
          scriptUrl: abp.appPath + 'view-resources/Areas/App/Views/OrganizationUnits/_AddDepartmentModal.js',
          modalClass: 'AddDepartmentModal',
          addDepartmentOptions: {
              title: app.localize('SelectADepartment'),
              serviceMethod: _organizationUnitService.findTenantDepartments,
          },
      });
    ...
    var organizationTree = {
      ...
      contextMenu: function (node) {
        var items = {
          ...
          addRole: {
            ...
            addDepartment: {
                label: app.localize('AddDepartment'),
                icon: 'la la-user-plus',
                _disabled: !_permissions.manageDepartments,
                action: function () {
      ...
      generateTextOnTree: function (ou) {
          var itemClass = ou.memberCount > 0 || ou.roleCount || ou.departmentCount ? ' ou-text-has-members' : ' ou-text-no-members';
        return (
          '<span title="' +
          ou.code +
          ...
          ' ' +
            app.localize('Roles') +
            ' ,</span> <span class="ou-text-department-count ml-1">' +
            ou.departmentCount +
            ' ' +
            app.localize('Departments') +
          '</span></span></span>'
        );
      },
      ...
      incrementRoleCount: function (ouId, incrementAmount) {
        ...
        incrementDepartmentCount: function (ouId, incrementAmount) {
            var treeNode = organizationTree.$tree.jstree('get_node', ouId);
            treeNode.original.departmentCount = treeNode.original.departmentCount + incrementAmount;
            organizationTree.$tree.jstree('rename_node', treeNode, organizationTree.generateTextOnTree(treeNode.original));
        },
    ...
    var roles = {
      ...
      var departments = {
          $table: $('#OuDepartmentsTable'),
          $emptyInfo: $('#OuDepartmentsEmptyInfo'),
          $addDepartmentButton: $('#AddDepartmentToOuButton'),
          $selectedOuRightTitle: $('#SelectedOuRightTitle'),
          //$deleteDepartmentButton: $('.delete-department-button'),
          dataTable: null,


          showTable: function () {
              departments.$emptyInfo.hide();
              departments.$table.show();
              departments.$addDepartmentButton.show();
              departments.$selectedOuRightTitle.text(organizationTree.selectedOu.displayName).show();
          },

          hideTable: function () {
              departments.$selectedOuRightTitle.hide();
              departments.$addRoleToOuButton.hide();
              departments.$table.hide();
              departments.$emptyInfo.show();
          },
          ...
          add: function (departmentList) {
              ...
              _organizationUnitService
                  .addDepartmentsToOrganizationUnit({
                      organizationUnitId: ouId,
                      tenantDepartmentIds: departmentIds,
                  })
                  .done(function () {
                      abp.notify.success(app.localize('SuccessfullyAdded'));
                      organizationTree.incrementDepartmentCount(ouId, departmentIds.length);
                      departments.load();
                  });
          },
          ...
          remove: function (department) {
              ...
              abp.message.confirm(
                  app.localize('RemoveDepartmentFromOuWarningMessage', department.name, organizationTree.selectedOu.displayName),
                  app.localize('AreYouSure'),
                  function (isConfirmed) {
                      if (isConfirmed) {
                          _organizationUnitService
                              .removeDepartmentFromOrganizationUnit({
                                  organizationUnitId: parseInt(ouId),
                                  tenantDepartmentId: department.id
                              })
                              .done(function () {
                                  abp.notify.success(app.localize('SuccessfullyRemoved'));
                                  organizationTree.incrementDepartmentCount(ouId, -1);
                                  departments.load(); // Refresh the department list
                              });
                      }
                  }
              );
          ...
          init: function () {
              ...
              this.dataTable = departments.$table.find('.organization-departments-table').DataTable({
                  paging: true,
                  serverSide: true,
                  processing: true,
                  deferLoading: 0,
                  responsive: false,
                  listAction: {
                      ajaxFunction: _organizationUnitService.getOrganizationUnitDepartments,
                      inputFilter: function () {
                          return { id: organizationTree.selectedOu.id };
                      },
                  },
                  columnDefs: [
                      {
                          ...
                          rowAction: {
                              ...
                              visible: function () {
                                  return _permissions.manageDepartments;
                              },
                          },
                      },
                      {
              ...
              $('#AddDepartmentToOuButton').click(function (e) {
                  e.preventDefault();
                  departments.openAddModal();
              });


              //departments.hideTable();
          },
      };
