...
{
    public class SurpathEntityHistoryHelper : EntityHistoryHelperBase, IEntityHistoryHelper, ITransientDependency
    {
        public SurpathEntityHistoryHelper(
            ...
            bool auditedPropertiesOnly)
        {
            ...
            foreach (var property in properties)
            {
                ...
                {
                    propertyChanges.Add(CreateEntityPropertyChange(
        ...
        private string DoMask(string? o)
        {
            try
            {
                if (string.IsNullOrEmpty(o)) return o;
                var _o = o; // .FromJsonString<string?>();
                if (_o != null)
                {
                    var _quotes = false;
                    if (_o.StartsWith('"') && _o.EndsWith('"'))
                    {
                        _o = _o.Substring(1);
                    ...
                    var result = regex.Replace(_o, "*");
                    _o = result;
                    if (_quotes) _o = '"' + _o + '"';
                }
                return _o; //.ToJsonString();
            }
            catch (Exception)
            {
                throw;
        ...
        private void UpdateChangeSet(EntityChangeSet changeSet)
        {
            ...
            foreach (var entityChange in changeSet.EntityChanges)
            {
                ...
                var hideValue = false;
                if (entityEntry.Entity.GetType() == typeof(UserPid))
                {
                    // check if pidtype maskpid is true, if so, set hideValue to true
                    var dbContext = (inzibackendDbContext)entityEntry.Context;
                    var _obj = (UserPid)entityEntry.Entity;
                    hideValue = dbContext.PidTypes.Where(p => p.Id == _obj.PidTypeId).Select(p => p.MaskPid).First();
                }
                ...
                foreach (var propertyChange in entityChange.PropertyChanges)
                {
                    ...
                    if (hideValue && property.Name.Equals("pid", StringComparison.InvariantCultureIgnoreCase))
                    {
                        var origValue = DoMask(propertyChange.OriginalValue.FromJsonString<string>());
                        var newValue = DoMask((string)propertyEntry.GetNewValue());
                        propertyChange.SetOriginalValue(origValue?.ToJsonString());
