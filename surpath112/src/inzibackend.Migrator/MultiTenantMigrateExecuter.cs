...
namespace inzibackend.Migrator
{
    public class MultiTenantMigrateExecuter : ITransientDependency
    {
        ...
        public void SeedFromSPL(IIocResolver iocResolver)
        {
            SeedHelper.Log.Logger = Log.Logger;
            SeedHelper.Log.ToConsole = true;
            SeedHelper.Log.ToFile = false;

            SeedHelper.SeedHostDb(iocResolver);
        }
    }
}