...
namespace inzibackend.Web.Url
{
    public abstract class AppUrlServiceBase : IAppUrlService, ITransientDependency
    {
        ...
        public string CreateViewCohortUserUrlFormat(int? tenantId)
        {
            var link = WebUrlService.GetSiteRootAddress(GetTenancyName(tenantId)).EnsureEndsWith('/') + "App/CohortUsers/ViewCohortUser/{cohortUserId}";
