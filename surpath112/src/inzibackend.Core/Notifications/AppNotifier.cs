...
{
    public class AppNotifier : inzibackendDomainServiceBase, IAppNotifier
    {
        ...
        public Task RecordStateChanged(UserIdentifier user, LocalizableString localizableMessage,
            IDictionary<string, object> localizableMessageData = null,
            NotificationSeverity severity = NotificationSeverity.Info)
        {
            return SendNotificationAsync(AppNotificationNames.RecordStateChanged, user, localizableMessage,
                localizableMessageData, severity);
        }
        public Task RecordStateExpirationWarning(UserIdentifier user, LocalizableString localizableMessage,
            IDictionary<string, object> localizableMessageData = null,
            NotificationSeverity severity = NotificationSeverity.Info)
        {
            return SendNotificationAsync(AppNotificationNames.RecordStateExpirationWarning, user, localizableMessage,
                localizableMessageData, severity);
        }
        public async Task RecordStateExpirationWarning(User user)
        {
            await _notificationPublisher.PublishAsync(
                AppNotificationNames.RecordStateExpirationWarning,
                new MessageNotificationData(L("RecordStateExpirationWarningNotificationMessage")),
                severity: NotificationSeverity.Success,
                userIds: new[] { user.ToUserIdentifier() }
            );
        }
        ...
        //public async Task YouHaveUnreadNotifications(User user)
        //{
        //    await _notificationPublisher.PublishAsync(
        //        AppNotificationNames.YouHaveUnreadNotifications,
        //        new MessageNotificationData(L("YouHaveUnreadNotifications")),
        //        severity: NotificationSeverity.Success,
        //        userIds: new[] { user.ToUserIdentifier() }
        //    );
        //}
