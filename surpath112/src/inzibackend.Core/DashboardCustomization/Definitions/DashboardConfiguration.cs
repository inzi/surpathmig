...
namespace inzibackend.DashboardCustomization.Definitions
{
    public class DashboardConfiguration
    {
        ...
        public DashboardConfiguration()
        {
            ...
            var surpathDeptFilter = new WidgetFilterDefinition(
                inzibackendDashboardCustomizationConsts.Filters.SurpathDeptFilter,
                "SurpathDeptFilter"
            );

            WidgetFilterDefinitions.Add(surpathDeptFilter);
            #endregion
            ...
            // Surscan Working Filter
            var surpathDept = new WidgetDefinition(
                inzibackendDashboardCustomizationConsts.Widgets.Tenant.SurpathDept,
                "SurpathDeptWidget",
                side: MultiTenancySides.Tenant,
                usedWidgetFilters: new List<string> { surpathDeptFilter.Id },
                permissionDependency: simplePermissionDependencyForTenantDashboard
            );

            var surpathCohortCompliance = new WidgetDefinition(
                inzibackendDashboardCustomizationConsts.Widgets.Tenant.SurpathCohortCompliance,
                "SurpathCohortComplianceWidget",
                side: MultiTenancySides.Tenant,
                permissionDependency: simplePermissionDependencyForTenantDashboard
            );
            ...
            // Surscan Working Filter
            WidgetDefinitions.Add(surpathDept);
            WidgetDefinitions.Add(surpathCohortCompliance);
            ...
            var defaultTenantDashboard = new DashboardDefinition(
                inzibackendDashboardCustomizationConsts.DashboardNames.DefaultTenantDashboard,
                new List<string>
                {
                    generalStats.Id, dailySales.Id, profitShare.Id, memberActivity.Id, regionalStats.Id, topStats.Id, salesSummary.Id, 
                    surpathDept.Id // Surscan Working Filter
                    ,surpathCohortCompliance.Id
                });

            DashboardDefinitions.Add(defaultTenantDashboard);

            var defaultHostDashboard = new DashboardDefinition(
                inzibackendDashboardCustomizationConsts.DashboardNames.DefaultHostDashboard,
                new List<string>
                {
                    incomeStatistics.Id,
                    hostTopStats.Id,
