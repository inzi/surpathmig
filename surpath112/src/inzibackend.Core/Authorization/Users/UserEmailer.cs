...
namespace inzibackend.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : inzibackendServiceBase, IUserEmailer, ITransientDependency
    {
            ...
            StringBuilder mailMessage)
        {
            ...
            string emailHtml = emailTemplate.ToString();
        ...
        /// <summary>
        ///  Sends a generic email to the user that a notification needs their attention
        /// </summary>
        ...
        public async Task SendEmailForComplianceRelatedNotification(User user, string link)
        {
            try
            {
                var emailTemplate = GetTitleAndSubTitle(user.TenantId, L("SendEmailForComplianceRelatedNotification_Title"),
                    L("SendEmailForComplianceRelatedNotification_SubTitle"));
                var mailMessage = new StringBuilder();

                mailMessage.AppendLine("<b>" + L("NameSurname") + "</b>: " + user.Name + " " + user.Surname + "<br />");

                mailMessage.AppendLine("<b>" + L("SendEmailForComplianceRelatedNotification_Message") + "<br />");
                ...
                //                       " UTC<br />");
                //mailMessage.AppendLine("<b>" + L("Message") + "</b>: " + chatMessage.Message + "<br />");
                //SendEmailForComplianceRelatedNotification_ClickTheLinkBelowToLoginToSurpath
                mailMessage.AppendLine("<br />");
                mailMessage.AppendLine(L("SendEmailForComplianceRelatedNotification_ClickTheLinkBelowToLoginToSurpath") + "<br /><br />");
                mailMessage.AppendLine("<a style=\"" + _emailButtonStyle + "\" bg-color=\"" + _emailButtonColor +
                                       "\" href=\"" + link + "\">" + L("Login") + "</a>");
                mailMessage.AppendLine("<br />");
                mailMessage.AppendLine("<br />");
                ...
                await ReplaceBodyAndSend(user.EmailAddress, L("SendEmailForComplianceRelatedNotification_Subject"), emailTemplate,
                    mailMessage);
            }
            catch (Exception exception)
            {
