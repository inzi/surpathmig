...
using inzibackend.MultiTenancy.Demo;
...
namespace inzibackend.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
        ...
        public async Task<int> CreateWithAdminUserAsync(
            ...
            bool isDonorPay = false,
            bool isDeferDonorPerpetualPay = false,
            int clientPaymentType = 0
            )
        {
            int newTenantId;
            long newAdminId;
            ...
            if (isInTrialPeriod && !subscriptionEndDate.HasValue)
            {
                throw new UserFriendlyException(LocalizationManager.GetString(
                    inzibackendConsts.LocalizationSourceName, "TrialWithoutEndDateErrorMessage"));
            }

            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {

                var _ClientPaymentType = isDonorPay == false ? EnumClientPaymentType.InvoiceClient : EnumClientPaymentType.DonorPays;
                if (isDeferDonorPerpetualPay) _ClientPaymentType = EnumClientPaymentType.DeferDonorPerpetualPay;

                //Create tenant
                var tenant = new Tenant(tenancyName, name)
                {
                    IsActive = isActive,
                    IsDonorPay = isDonorPay,
                    IsDeferDonorPerpetualPay = isDeferDonorPerpetualPay,
                    ClientPaymentType = _ClientPaymentType,
                    EditionId = editionId,
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                ...
                var error = LocalizationManager.GetSource(inzibackendConsts.LocalizationSourceName)
        ...
        public override Task<Tenant> FindByTenancyNameAsync(string clientCode)
        {
            return Task.Run(async () =>
            {
                var t = await _tenantRepository.FirstOrDefaultAsync(t => t.TenancyName == clientCode);
                if (t == null)
                {
                    t = await _tenantRepository.FirstOrDefaultAsync(t => t.ClientCode.ToUpper().Equals(clientCode.ToUpper()));
                }
                return t;
            });
        }

        public async Task<Dictionary<int, string>> GetTenantNames()
        {
            var t = await _tenantRepository.GetAllListAsync();
            var r = t.Select(x => new { x.Id, x.Name }).ToDictionary(x => x.Id, x => x.Name);
            return r;


        }

        public async Task<Dictionary<int, string>> GetTenancyNames()
        {
            var t = await _tenantRepository.GetAllListAsync();
            var r = t.Select(x => new { x.Id, x.TenancyName }).ToDictionary(x => x.Id, x => x.TenancyName);
            return r;
        }

        public async Task<Dictionary<int, Tuple<int,string,string>>> GetTenancyInfoList(int[] idlist = null)
        {
            var t = await _tenantRepository.GetAllListAsync();
            if (idlist!=null && idlist.Length>0)
            {
                t = t.Where(x => idlist.Contains(x.Id)).ToList();
            };
            var r = t.Select(x => new Tuple<int, string, string>(x.Id, x.Name, x.TenancyName)).ToDictionary(x => x.Item1, x => x);
            return r;
        }
    }
}
