...
using inzibackend.Auditing.Exporting;
using inzibackend.Authorization;
using inzibackend.Authorization.Users;
using inzibackend.Dto;
using inzibackend.EntityHistory;
using EntityHistoryHelper = inzibackend.EntityHistory.EntityHistoryHelper;

namespace inzibackend.Auditing
{
    [DisableAuditing]
    [AbpAuthorize(AppPermissions.Pages_Administration_AuditLogs)]
    public class AuditLogAppService : inzibackendAppServiceBase, IAuditLogAppService
    {
        ...
        private readonly IEntityPropertyValueResolver _entityPropertyValueResolver;
        ...
        public AuditLogAppService(
            ...
            IEntityPropertyValueResolver entityPropertyValueResolver)
        {
            ...
            _entityPropertyValueResolver = entityPropertyValueResolver;
        }
        ...
        public async Task<List<EntityPropertyChangeDto>> GetEntityPropertyChanges(long entityChangeId)
        {
            ...
            var propertyChangeDtos = ObjectMapper.Map<List<EntityPropertyChangeDto>>(entityPropertyChanges);

            // Get the entity change to get the entity type for context
            var entityChange = await _entityChangeRepository.GetAsync(entityChangeId);

            // Resolve user-friendly descriptions for each property change
            foreach (var dto in propertyChangeDtos)
            {
                // Resolve original value description
                if (!string.IsNullOrWhiteSpace(dto.OriginalValue))
                {
                    dto.OriginalValueDescription = await _entityPropertyValueResolver
                        .ResolvePropertyValueAsync(entityChange.EntityTypeFullName, dto.PropertyName, dto.OriginalValue);
                }

                // Resolve new value description
                if (!string.IsNullOrWhiteSpace(dto.NewValue))
                {
                    dto.NewValueDescription = await _entityPropertyValueResolver
                        .ResolvePropertyValueAsync(entityChange.EntityTypeFullName, dto.PropertyName, dto.NewValue);
                }
            }

            return propertyChangeDtos;
        }
