    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        public async Task<TenantSettingsEditDto> GetAllSettings()
        {
            var settings = new TenantSettingsEditDto
            {
                ...
                PaymentPopupSettings = await GetPaymentPopupSettingsAsync(),
        ...
        private async Task<TenantPaymentPopupSettingsEditDto> GetPaymentPopupSettingsAsync()
        {
            var enablePaymentPopupForCohort = await SettingManager.GetSettingValueForTenantAsync(AppSettings.PaymentPopup.EnablePaymentPopupForCohort, AbpSession.GetTenantId()) ?? "";
            var enablePaymentPopupForDepartment = await SettingManager.GetSettingValueForTenantAsync(AppSettings.PaymentPopup.EnablePaymentPopupForDepartment, AbpSession.GetTenantId()) ?? "";
            
            return new TenantPaymentPopupSettingsEditDto
            {
                EnablePaymentPopupForCohort = enablePaymentPopupForCohort,
                EnablePaymentPopupForDepartment = enablePaymentPopupForDepartment
            };
        }
        ...
        public async Task UpdateAllSettings(TenantSettingsEditDto input)
        {
            ...
            await UpdatePaymentPopupSettingsAsync(input.PaymentPopupSettings);
        ...
        {
            if (_multiTenancyConfig.IsEnabled && !inzibackendConsts.AllowTenantsToChangeEmailSettings)
            {
                return;
            }
        ...
        private async Task UpdatePaymentPopupSettingsAsync(TenantPaymentPopupSettingsEditDto settings)
        {
            await SettingManager.ChangeSettingForTenantAsync(
                AbpSession.GetTenantId(),
                AppSettings.PaymentPopup.EnablePaymentPopupForCohort,
                settings.EnablePaymentPopupForCohort
            );
            
            await SettingManager.ChangeSettingForTenantAsync(
                AbpSession.GetTenantId(),
                AppSettings.PaymentPopup.EnablePaymentPopupForDepartment,
                settings.EnablePaymentPopupForDepartment
            );
        }
