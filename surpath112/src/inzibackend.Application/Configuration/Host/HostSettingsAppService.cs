...
namespace inzibackend.Configuration.Host
{
    ...
    public class HostSettingsAppService : SettingsAppServiceBase, IHostSettingsAppService
    {
        ...
        private async Task<PaymentPopupSettingsEditDto> GetPaymentPopupSettingsAsync()
        {
            var enableGlobalPaymentPopup = await SettingManager.GetSettingValueForApplicationAsync(AppSettings.PaymentPopup.EnableGlobalPaymentPopup);
            
            return new PaymentPopupSettingsEditDto
            {
                EnableGlobalPaymentPopup = enableGlobalPaymentPopup.ToLower() == "true"
            };
        }
        ...
        private async Task UpdatePaymentPopupSettingsAsync(PaymentPopupSettingsEditDto settings)
        {
            await SettingManager.ChangeSettingForApplicationAsync(
                AppSettings.PaymentPopup.EnableGlobalPaymentPopup,
                settings.EnableGlobalPaymentPopup.ToString().ToLowerInvariant()
            );
        }
