...
namespace HL7ParserService
{
    public class HL7Processor
    {
        ...
        public BackendParserReportHelper ReportHelper;
        public Process proc;
        public string JsonParserMasks;
        ...
        public HL7Processor(ILogger __logger)
        {
        ...
        // This replaces MAIN from program in command line version
        public bool main()
        {
            var _retval = false;
            try
            {
                ...
                _logger.Debug($"Download files from FTP? {DoFTP}");
                ...
                IntegrationPusher integrationPusher = new IntegrationPusher(false, _logger);
                ...
                {
                    // this will generate HL7 and MRO files for staging.
                    HL7Stage h = new HL7Stage(_logger);
                    h.Gen();
                }
                else
                {
                    _logger.Debug("Is production, not generating stage files");
                }
        ...
        private bool ProcessCRLLabFiles()
        {
            bool complete = false;
            try
            {
        ...
        private bool ProcessQuestLabFiles()
        {
            bool complete = false;
            try
            {
        ...
        private void DownloadFiles(string sourcePath, string destPath, ref List<string> fileList, string ftpHost, string ftpUser, string ftpPassword, int ftpPort)
        {
        ...
        private void DeleteFiles(string sourcePath, ref List<string> fileList, string ftpHost, string ftpUser, string ftpPassword, int ftpPort)
        {
            if (DoFTP)
            {
                List<string> dlfileList = new List<string>();
        ...
        private void UploadFiles(string sourcePath, string destPath, string ftpHost, string ftpUser, string ftpPassword, int ftpPort)
        {
            if (DoFTP)
            {
                JSch jsch = new JSch();
