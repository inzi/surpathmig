...
namespace ProjectConcertTestCLI
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            logger.Information("Project Concert Test CLI - HL7 Stage Generator");
            logger.Information("================================================");
            ...
            try
            {
                // Get connection string from config - prioritize AppSettings like other components
                string connectionString = ConfigurationManager.AppSettings["ConnectionString"] 
                    ?? ConfigurationManager.ConnectionStrings["SurpathConnection"]?.ConnectionString;
                ...
                using (var conn = new MySqlConnection(connectionString))
                {
                    ...
                    if (donorId.HasValue && resetStatus)
                    {
                        ...
                        string updateQuery = @"
                            ...
                            SET test_status = 4,
                                last_modified_by = 'ProjectConcertTestCLI',
                    ...
                    // Show count of donors that will be processed
                    string countQuery = @"
                ...
                if (donorId.HasValue && resetStatus && args.Contains("--reset-after"))
                {
                    using (var conn = new MySqlConnection(connectionString))
                    {
                        ...
                        string resetQuery = @"
                            UPDATE donor_test_info 
                            SET test_status = 6,
                                last_modified_by = 'ProjectConcertTestCLI-Reset',
                                last_modified_on = NOW()
                            WHERE donor_id = @donorId
                            AND test_status = 4
                            ORDER BY donor_test_info_id DESC
        ...
        {
            Console.WriteLine("Usage: ProjectConcertTestCLI.exe <donor_id> [options]");
            Console.WriteLine("       ProjectConcertTestCLI.exe --generate-only");
            Console.WriteLine();
            ...
            Console.WriteLine("  ProjectConcertTestCLI.exe 12345");
            Console.WriteLine("  ProjectConcertTestCLI.exe 12345 --reset-after");
            Console.WriteLine("  ProjectConcertTestCLI.exe --generate-only");
        }
    }
}