...
namespace SurPath.MySQLHelper
{
    ...
    {
        public ILogger _logger { get; set; } = new LoggerConfiguration().CreateLogger();

        #region Private utility methods & constructors

        // Since this class provides only static methods, make the default constructor private to prevent Command
        ...
        private static void PrepareCommand(MySqlCommand command, MySqlConnection connection, MySqlTransaction transaction, CommandType commandType, string commandText, MySqlParameter[] commandParameters, out bool mustCloseConnection)
        {
            ...
            if (transaction != null)
            {
                ...
                {
                    Debug.WriteLine("SqlException: The tranaction was rolled back.");
        ...
        public static int ExecuteNonQuery(MySqlTransaction transaction, CommandType commandType, string commandText, params MySqlParameter[] commandParameters)
        {
            ...
            Debug.WriteLine("SQL ExecuteNonQuery- " + commandText);
            foreach (var param in commandParameters)
            {
                try
                {
                    if (param != null) Debug.WriteLine(param.ParameterName + ":" + param.Value);
                }
                catch (System.Exception ex)
                {
                    Debug.WriteLine("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
                    Debug.WriteLine("|||||||||||||||||||||||||||||||||||||||||||||||||||||");
                    Debug.WriteLine("Error in Param: " + param.ParameterName);
                }
            }
        ...
        public static DataSet ExecuteDataset(MySqlConnection connection, CommandType commandType, string commandText, params MySqlParameter[] commandParameters)
        {
            ...
            try
            {
                if (commandParameters != null)
                {
                    foreach (MySqlParameter prm in commandParameters)
                    {
                        Debug.WriteLine("prm: " + prm.ParameterName + prm.Value);
                    }
                }
            }
            catch (Exception)
